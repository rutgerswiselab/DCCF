...
def uniform_expo_split(data_df, user2id, item2id, ratio):
    ...
    for i in tqdm(data_df['userId'].unique()):
        ...
        sampled_index = group_df.sample(n=treat_num).index.tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Electronics_15_15_uni', help='the name of dataset')
...
#     pdb.set_trace()
    print('splitting test set randomly')
    remain_df, test_df = uniform_expo_split(data_df, user2id, item2id, args.test_ratio)
    print('splitting validation set randomly')
    train_df, val_df = uniform_expo_split(data_df, user2id, item2id, args.val_ratio/(1-args.test_ratio))
